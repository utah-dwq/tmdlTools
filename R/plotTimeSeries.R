...
plotTimeSeries <- function(x,max_crit, sitepal = "basel"){
  # Get number of sites
  uni.sites <- unique(x$MLID)
  ...
  plot(1, type="n", xlab="Time", ylab="MPN/100 mL", xaxt="n", xlim=c(min.dat, max.dat), ylim=c(min.e, max.e))
  ...
  for(i in 1:length(uni.sites)){
    y = x[x$MLID==uni.sites[i],]
    lines(y$E.coli~y$Date, lwd=1, lty=2, col=colrs[i])
    points(y$E.coli~y$Date, pch=21, col="black", bg=colrs[i])
