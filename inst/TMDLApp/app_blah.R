...
server <- function(input, output) {
  ...
  output$unit_type <- renderUI({
    subdat = colnames(month.dat[month.dat$ML_Name==input$site2,])
    subd = subdat[subdat%in%c("Observed_Loading","E.coli_Geomean")]
    ...
    output$Monthly_Geomeans <- renderPlot({
       ...
       rownames(x) <- x$month
       uplim = max(x$Observed_Loading,x$Loading_Capacity_MOS)*1.2
       ...
       if(input$medplot){
         ...
         boxplot(datstack$Loading~datstack$Meas_Type+lubridate::month(datstack$Date),
                 lty=1, xaxt="n", frame=FALSE, col=alpha(c("firebrick3", "dodgerblue3"),0.1), boxwex = 0.7, at=ax_spots, add=TRUE)
       }
